...
const [data, setData] = useState([]);
  ...
  const imageURL = (path) => {
    ...
    return `https://befikr.in/Testimonial/${filename}`;
  };

  const fetchJobs = () => {
    fetch("https://befikr.in/testimonial_api.php")
      .then((res) => {
  ...
  return (
    <div className="relative flex w-full p-4 justify-center">
      ...
      <AnimatePresence>
        {hoveredIndex !== null && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <motion.div className="text-black text-center flex h-3/4 w-3/4" initial={{ y: 20 }} animate={{ y: 0 }} exit={{ y: 20 }}>
              <div className="flex-1 mr-12">
                <img
                  src={imageURL(data[hoveredIndex]?.image)}
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 justify-items-center items-center gap-4">
        {data.map((item, index) => (
          <div
            key={index}
          ...
          >
            <img
              ...
              alt={item.name}
              className={`block w-40 h-40 m-4 rounded-full object-cover transition-all duration-300 ${
                hoveredIndex === index ? 'opacity-0 z-50' : ''
              }`}
            />
            <h2 className="text-gray-700 font-semibold m-2 mb-0">{item.name}</h2>

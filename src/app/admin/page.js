...
const AdminDashboard = () => {
  ...
  const mostActiveJob = Object.entries(applicants).reduce(
    (max, [jobId, apps]) =>
      apps.length > max.count
        ? { title: apps[0]?.position || "Unknown", count: apps.length }
        : max,
    { title: null, count: 0 }
  );
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50 p-8 font-sans">
        ...
        {/* Jobs Table */}
        <div className="flex flex-wrap gap-3 w-full md:w-full mb-8">
              ...
              <tbody>
                {paginatedJobs.map((job, idx) => (
                  ...
                  >
                    ...
                    <td
                      className={`py-3 px-4 font-semibold ${job.status === "active" ? "text-green-600" : "text-red-600"
                        }`}
                    >
